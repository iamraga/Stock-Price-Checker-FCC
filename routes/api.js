      ...
      if (Array.isArray(stocks)) { //two stocks
        ...
        Promise.all([stock1Promise, stock2Promise])
          .then(values => {
            ...
            if(!values[0].data["Global Quote"] || !values[1].data["Global Quote"]) return res.send("there was a problem");
            if (!values[0].data["Global Quote"].hasOwnProperty("01. symbol")) return res.send("stock " + stocks[0] + " doesn't exist");
            else if (!values[1].data["Global Quote"].hasOwnProperty("01. symbol")) return res.send("stock " + stocks[1] + " doesn't exist");

            Stock.findOne({stockName: values[0].data["Global Quote"]["01. symbol"]}, (err, stockDoc1) => {
              if (err) return res.send("There was an error");
              else if (stockDoc1) { //Modify existing stock on the db
                stockDoc1.stockPrice = values[0].data["Global Quote"]["05. price"];//Update stock's price
                likeStock(stockDoc1, like, ip);
                stockDoc1.save(saveFirstStock(res, values[1].data, like, ip));
              } 
              else //Creates a new stock into the db
                new Stock(createStock(values[0].data, like, ip))
                  .save(saveFirstStock(res, values[1].data, like, ip));
            });
          });;
      } 
      else { //Only one stock
        axios.get("https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=" + stocks + "&apikey=" + process.env.ALPHA_VANTAGE_KEY)
          .then(response => {
            //If stock doesn't exist
            if (!response.data["Global Quote"].hasOwnProperty("01. symbol")) return res.send("stock " + stocks + " doesn't exist");
            //Stock exists on alpha vantage db
            Stock.findOne({stockName: response.data["Global Quote"]["01. symbol"]}, (err, stockDoc) => {
              if (err) return res.send("There was an error");
              else if (stockDoc) { //Modify existing stock on the db
                stockDoc.stockPrice = response.data["Global Quote"]["05. price"]; //Update stock's price
                likeStock(stockDoc, like, ip);
                stockDoc.save(saveSingleStock(res));
              } 
              else //Create a new stock on the db
                new Stock(createStock(response.data, like, ip)).save(saveSingleStock(res));
            });
          })
...
const saveFirstStock = (res, data, like, ip) => (err, s1) => {
  if (err) return res.send("error saving the document");
    ...
    Stock.findOne({stockName: data["Global Quote"]["01. symbol"]}, (err, stockDoc2) => {
      if (err) return res.send("There was an error");
      else if (stockDoc2) { //Modify existing stock on the db
...
const likeStock = (stockDoc, like, ip) => {
  ...
  //Retire like from stock
    ...
    stockDoc.stockLikesIp = [...stockDoc.stockLikesIp.slice(0, ipIndex), ...stockDoc.stockLikesIp.slice(ipIndex + 1)];
  }
};
